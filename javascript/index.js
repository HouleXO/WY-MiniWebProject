...
function setData(num,element){
	...
	// 课程HTML结构设置
	function reClassData(data){   //设置课程
	...
	function setCourse(data){
		...
		*/
		...
		/*EventUtil.addHandler(window, 'resize', function(event){
			...
			if( getWindowWidth() < 1205 ){
				classData.psize = 15 ;
				get('http://study.163.com/webDev/couresByCategory.htm',classData,reClassData);
			}else{
				classData.psize = 20 ;
				get('http://study.163.com/webDev/couresByCategory.htm',classData,reClassData);
			}
...
function setData(num,element){
	...
	var	psize = 20;
	var	type = num;
	GLOBAL.COURSE.pageNo = pageNo;
	GLOBAL.COURSE.psize = psize;
	GLOBAL.COURSE.type = type;
	GLOBAL.COURSE.setCourse = setCourse;
		
	function setCourse(data){
		get('http://study.163.com/webDev/couresByCategory.htm',{pageNo:GLOBAL.COURSE.pageNo,psize:GLOBAL.COURSE.psize,type:GLOBAL.COURSE.type},function(data){   //设置课程
			var data = JSON.parse(data);			
			for( var i=0; i<data.list.length; i++){
				var oTeam = document.createElement('div');
				oTeam.className = 'course-details';
...
function pageChange(){
	...
	oPageLeft.onclick = function(){
		if(GLOBAL.COURSE.pageNo>1){
			aPage[GLOBAL.COURSE.pageNo-1].className = 'page-num';
			aPage[GLOBAL.COURSE.pageNo-1].className = 'active';
			GLOBAL.COURSE.pageNo--;
	...
	for(var i = 0; i < aPage.length; i++){
		...
		aPage[i].onclick = function(){
			if((GLOBAL.COURSE.pageNo-1) != this.index){
				for(var i = 0; i < aPage.length; i++){
					aPage[i].className = 'page-numLk';
				}
				GLOBAL.COURSE.pageNo = this.index+1;					
			}
			setData(10,aDesign[0]);
	        setData(20,aLanguage[0]);
		}			
	...
	for(var i = 0; i < aPage.length; i++){
		...
		aPage[i].onclick = function(){
			if((data.pagination.pageIndex-1) != this.index){
				for(var i = 0; i < aPage.length; i++){
					aPage[i].className = 'page-num';
				}
				data.pagination.pageIndex = this.index+1;
				this.className = 'page-num active';
				get({
				    url : 'http://study.163.com/webDev/couresByCategory.htm',
				    data : {
				        'pageNo':data.pagination.pageIndex,
